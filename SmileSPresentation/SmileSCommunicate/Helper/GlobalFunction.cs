...
namespace SmileSCommunicate.Helper
{
    public class GlobalFunction
    {
        ...
        public static async Task<SMS_DetailResult> SendSMSV2(string phoneNo, string message, int smsTypeId, int createdById)
        {
            var request = new RestRequest($"{Properties.Settings.Default.EndpointSMS_API}/SendSMSV2", Method.POST);

            var requestData = new SMSSendV2RequestDto
            {
                PhoneNo = phoneNo,
                Message = message,
                SMSTypeId = smsTypeId,
            ...
            var output = JsonConvert.DeserializeObject<SMS_DetailResult>(response.Content);

            return output;
        }

        public static async Task<SMS_DetailResult> SendSMSWithRef(string phoneNo, string message, string @ref, int smsTypeId, int createdById)
        {
            var request = new RestRequest($"{Properties.Settings.Default.EndpointSMS_API}/SendSMSWithRef", Method.POST);

            var requestData = new SendSMSWithRefRequestDto
            {
                Reference = @ref,
                PhoneNo = phoneNo,
                Message = message,
