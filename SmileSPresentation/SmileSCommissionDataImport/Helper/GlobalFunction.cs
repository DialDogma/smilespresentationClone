...
namespace SmileSCommissionDataImport.Helper
{
    public class GlobalFunction
    {
        ...
        public static Result GetLatestPeriod()
        {
            ...
            using (var db = new CommissionCalculateEntities())
            {
                var listResult = db.usp_CommissionPeriod_GetLastedPeriod_Select().FirstOrDefault();
                if (listResult != null && listResult.CommissionPeriodStatusId == 2)
                {
                    result.periodId = listResult.PeriodId;
                    result.IsResult = true;
                }
            }
