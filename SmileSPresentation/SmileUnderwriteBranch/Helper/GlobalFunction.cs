...
namespace SmileUnderwriteBranch.Helper
{
    public class GlobalFunction
    {
        ...
        /// <summary>
        /// INSERT LOG BY CUTEFUL
        /// </summary>
        public static void PHQueueAnalytic_Insert(HttpContextBase context)
        {
            using (var db = new UnderwriteBranchEntities())
            {
                //GET LOGIN DETAIL
                var userId = GetLoginDetail(context).User_ID;
                //GET CURRENT URL
        ...
        {
            using (SqlConnection conn = new SqlConnection(new UnderwriteBranchEntities().Database.Connection.ConnectionString))
            {
                conn.Open();
