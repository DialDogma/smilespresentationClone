...
jQuery.validator.addMethod("checkStartDate", function (value, element) {
    var endDateId = $(element).attr('endDate');
    var endDate = $(`#${endDateId}`).val();

    var checkErrorBtnId = $(element).attr('checkErrorBtn');
    var checkErrorBtn = $(`#${checkErrorBtnId}`);

    var dateStartString = value;
    var [day, month, year] = dateStartString.split("/");
    var dateStart = new Date(`${year}-${month}-${day}`);

    var dateEndString = endDate;
...
}, "วันที่เริ่มต้นใม่ถูกต้อง");

jQuery.validator.addMethod("checkEndDate", function (value, element) {
    var startDateId = $(element).attr('startDate');
    var startDate = $(`#${startDateId}`).val();

    var checkErrorBtnId = $(element).attr('checkErrorBtn');
    var checkErrorBtn = $(`#${checkErrorBtnId}`);

    var dateEndString = value;
    var [day, month, year] = dateEndString.split("/");
    var dateEnd = new Date(`${year}-${month}-${day}`);

    var dateStartString = startDate;
    var [day, month, year] = dateStartString.split("/");
    var dateStart = new Date(`${year}-${month}-${day}`);
