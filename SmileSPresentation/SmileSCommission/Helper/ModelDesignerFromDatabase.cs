...
namespace SmileSCommission.Helper
{
    public class ModelDesignerFromDatabase
    {
        ...
        public ModelDesignerFromDatabase(int designerModel_Id)
        {
            ...
            //Get last period id
            Period_Id = _context.CommissionPeriod.OrderByDescending(x => x.Id).FirstOrDefault().Id;
            this.DesignerModel_Id = designerModel_Id;
            this.DesignerModel_Name = _context.DesignerModel.FirstOrDefault(x => x.Id == designerModel_Id).Name;
        }
        ...
        private bool CreateDatasourceSheet()
        {
            ...
            {
                // Get Datasource list
                var dsList = db.DesignerModelDatasource.Where(x => x.DesignerModel_Id == this.DesignerModel_Id).OrderByDescending(x => x.IsMainKey).ToList();

                //Loop in list
                for (int i = 0; i < dsList.Count; i++)
                {
                    //Create Sheet
                    var sheet = ExcelModelDesigner.Workbook.Worksheets.Add(dsList[i].DatasourceName);
                    //Bind Sheet
                    if (!BindDatasourceSheet(sheet, dsList[i].DatasourceName))
                    {
                        this.IsValid = false;
                        this.ErrorText = "Error on creating sheet:" + dsList[i].DatasourceName;
                        return false;
                    }
                }
        ...
        /// <returns></returns>
        private bool BindDatasourceSheet(ExcelWorksheet sheet,string datasourceName)
        {
            //Bind Sheet
            ...
            command.Parameters.Add(new SqlParameter("TableName", datasourceName));
            ...
            try
            {
                ...
                // add null value if rows count = 0 
                if (dt.Rows.Count == 0)
                {
                    dt.Rows.Add();
                }
                ...
                for (int i = 1; i < dt.Columns.Count + 1; i++)
                {
                    ...
                    //gat name
                    var name = datasourceName + "_" + sheet.Cells[1, i].Text;
                    //Naming Range
                ...
                var tableNamingRange = new ExcelNamedRange(datasourceName, null, sheet, tableAddress, 1);
                sheet.Workbook.Names.Add(datasourceName, tableNamingRange);



                //Naming for HeaderColumns
                var colHeaderAddress = sheet.Cells[1, 1, 1, dt.Columns.Count].Address;
                var colHeaderName = datasourceName + "_HeaderColumns";
                var colHeaderNamingRange = new ExcelNamedRange(colHeaderName, null, sheet, colHeaderAddress, 1);
                sheet.Workbook.Names.Add(colHeaderName, colHeaderNamingRange);
        ...
        {
            bool result = false;
            //var db = new ModelCalculationIndex();
            ...
            try
            {
                ...
                var mainIndexSheetName = _context.DesignerModelDatasource.Where(x => x.DesignerModel_Id == this.DesignerModel_Id && x.IsMainKey == true).FirstOrDefault().DatasourceName;

                //Get MainIndex last row
                var mainIndexSheet = ExcelModelDesigner.Workbook.Worksheets[mainIndexSheetName];
                var dt = cExcelHelper.GetDataTableFromExcel(mainIndexSheet, true);
                var mainIndexSheetRowsCount = dt.Rows.Count;

                //Create Parameter
                var lstParameter = _context.DesignerParameter.Where(x => x.DesignerModel_Id == this.DesignerModel_Id).OrderBy(x => x.ParameterIndex).ToList();
                ...
                //Create Naming for Header Columns
                var headerColumnAddress = sheet.Cells[1, 1, 1, lstParameter.Count].Address;
                var headerColumnNameRange = new ExcelNamedRange("HeaderColumns", null, sheet, headerColumnAddress, 1);
                sheet.Workbook.Names.Add("HeaderColumns", headerColumnNameRange);
