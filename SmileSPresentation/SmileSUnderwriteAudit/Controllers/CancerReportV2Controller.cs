...
{
    [Authorization]
    public class CancerReportV2Controller : Controller
    {
        private readonly UnderwriteCancerAuditEntities _context;

        public CancerReportV2Controller()
        {
        ...
        public ActionResult CancerAuditInsureReportCheckPeriod(string period)
        {
            try
            {
                ...
                DateTime? oldPeriod = Convert.ToDateTime("2567-09-01");

                if (nPeriod >= oldPeriod)
                {
                    //รายงานตัวใหม่
                    return CancerAuditInsureReportV2(period);
                }
                else
                {
                    //รายงานตัวเก่า
                    return CancerAuditInsureReportV1(period);
                }
            }
            catch (Exception e)
            {
        ...
        public ActionResult CancerAuditInsureReportV1(string period)
        {
            try
            {
        ...
        public ActionResult CancerAuditInsureReportV2(string period)
        {
            try
            {
                var a = Convert.ToDateTime(period);
                ...
                using (var package = new ExcelPackage(stream))
                {
                    var workSheet = package.Workbook.Worksheets.Add("Detail");
                    {
                        var auditMotor = result.Select(o => new
                        {
                            ...
                            วันที่ทำรายการ = o.AuditQCUpdatedDate,
                            เวลา = o.AuditQCUpdatedTime,
